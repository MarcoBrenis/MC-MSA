...
def clear_cache_dir(target_dir: Path, description: str):
    ...
    if target_dir.exists():
        ...
        print(f" [CACHE FLUSH] Eliminada carpeta de caché: {target_dir} ({description})")
    else:
        print(f" [CACHE INFO] La carpeta {target_dir} ya estaba vacía.")
...
def run_phase1_extraction(
    ...
    """
    FASE 1: Extrae contornos de pitch (f0), tiempos y energía.
    Guarda los resultados en cache_ciarp/phase1_f0/{method}/{filename}.json
    """
    print("\n" + "=" * 80)
    print(" FASE 1: Extracción y Caché de Características de Melodía (f0 Pitch)")
    print("=" * 80)
    ...
    for method in methods:
        ...
        for file_key, file_path in all_files.items():
            ...
            if features is None:
                print(f"   [FASE 1 - Extrayendo] {method} | {file_path.name}...")
        ...
        print(f" [FASE 1 COMPLETADA] Método: {METHOD_DISPLAY_NAMES.get(method, method)}")
...
def run_phase2_matrix_generation(
    ...
    """
    FASE 2: Lee f0 de Fase 1, realiza la segmentación formal (SSM / Algoritmo 1) y
    calcula la matriz de alineamiento DTW y similitudes NLCS par a par.
    Guarda las representaciones en cache_ciarp/phase2_matrices/{method}/{pair_key}.json
    """
    print("\n" + "=" * 80)
    print(" FASE 2: Generación de Matriz SSM, Segmentación y Alineamientos DTW")
    print("=" * 80)
    ...
    for method in methods:
        print(f"\n---> [FASE 2] Procesando Matrices y SSM para Método: {METHOD_DISPLAY_NAMES.get(method, method)}...")
        ...
        print(f" [FASE 2 COMPLETADA] Matrices y SSM listos para {METHOD_DISPLAY_NAMES.get(method, method)}")
...
def run_phase3_classification_reporting(
    ...
    """
    FASE 3: Lee las representaciones de Fase 2, ejecuta la evaluación del clasificador
    (MRR, Top-5, NLCS, CIs de Bootstrap, Grid Search de Matriz de Confusión), y exporta
    las tablas oficiales de resultados (LaTeX Tabla 2, Tabla 3 y resumen en texto).
    """
    print("\n" + "=" * 80)
    print(" FASE 3: Clasificador, Búsqueda de Umbral y Generación de Tablas CIARP")
    print("=" * 80)
    ...
    for method in methods:
        print(f"\n---> [FASE 3] Calculando métricas finales y CIs para: {METHOD_DISPLAY_NAMES.get(method, method)}...")
    ...
    print(f"\n========================================================")
    print(f" BENCHMARK COMPLETO - TABLAS FINALES DE RESULTADOS")
    print(f"========================================================\n")
...
def main():
    ...
    if args.interactive:
        print("\n========================================================")
        print(" MC-MSA CIARP 2026 - Opciones de Caché por Fases")
        print("========================================================")
        print(" [1] Borrar Caché FASE 1 (Extracción de F0)")
        print(" [2] Borrar Caché FASE 2 (Matriz SSM y Alineamientos DTW)")
        print(" [3] Borrar Caché FASE 3 (Resultados del Clasificador)")
        print(" [4] Borrar TODO el Caché (Fases 1, 2 y 3)")
        print(" [5] Ejecutar Fase 1 (Extracción de F0)")
        print(" [6] Ejecutar Fase 2 (Construcción de SSM y Alineamientos)")
        print(" [7] Ejecutar Fase 3 (Clasificador y Tablas Finales)")
        ...
        if choice == "1":
            ...
            clear_cache_dir(phase_dirs["phase2"], "Fase 2: SSM matrices")
            return
        elif choice == "3":
    ...
    if args.phase in ["1", "all"]:
        ...
        if args.phase == "1":
            print("\n[FASE 1 COMPLETADA] Las características f0 se han guardado en caché.")
            return
    ...
    if args.phase in ["2", "3", "all"]:
        ...
        if args.phase == "2":
            print("\n[FASE 2 COMPLETADA] Las matrices SSM y alineamientos se han guardado en caché.")
            return
