...
def escape_latex(text):
    ...
    for original, new_val in replacements.items():
        text = text.replace(original, new_val)
    return text
...
def extract_dataset_to_csv(dataset_dir, is_academic, csv_path):
    ...
    ruta_txt = Path(csv_path).with_suffix('.txt')
...
def generate_latex_table(csv_path, output_path, section_title, table_label, description):
    """
    ...
    has_author = "academic" in table_label or "acad" in csv_path.lower()
...
\\caption{{List of tracks included in the {section_title}.}} \\label{{{table_label}}} \\\\
\\toprule
...
"""
    ...
    for tid in sorted(tracks_data.keys()):
        ...
        work_title = escape_latex(orig_row.get('work title', 'Unknown'))
        work_artist = escape_latex(orig_row.get('work artist', 'Unknown'))
        orig_perf = escape_latex(orig_row.get('performance artist', 'Unknown'))
        cover_perf = escape_latex(cover_row.get('performance artist', 'Unknown'))
    ...
    with open(output_path, mode='w', encoding='utf-8') as archivo_salida:
        archivo_salida.write(latex_code)
        
    print(f"Success! Table generated at: {output_path}")

def process_single_dataset(ds_path):
    ds_name = ds_path.name
    print(f"\nProcessing dataset: {ds_name}...")
    
    is_academic = "acad" in ds_name.lower()
    
    confirm = input(f"Is this a classical/academic music dataset? (y/N) [Default: {'Yes' if is_academic else 'No'}]: ").strip().lower()
    ...
    ok = extract_dataset_to_csv(ds_path, is_academic=is_academic, csv_path=csv_name)
    if ok:
        section_title = ds_name.replace("dataset_", "").replace("_", " ").title() + " Corpus"
        if is_academic:
            description = f"This section details the works from the {section_title}."
        else:
            description = f"The following table lists the tracks evaluated in the {section_title}."
            
        generate_latex_table(
            csv_path=csv_name,
            output_path=tex_name,
            section_title=section_title,
            table_label=f"tab:{ds_name.replace('dataset_', '')}",
            description=description
        )

def process_defaults(base_path):
    # --- POPULAR CORPUS ---
    ...
    if academico_ok:
        ...
        generate_latex_table(
            ...
            description=texto_academico
        )

def dataset_selection_menu():
    ...
    while True:
        try:
            choice = input(f"Select an option (1-{len(datasets) + 2}): ").strip()
            if not choice:
                continue
            idx = int(choice)
            if 1 <= idx <= len(datasets):
                ds_seleccionado = datasets[idx - 1]
                process_single_dataset(base_path / ds_seleccionado)
                break
            elif idx == len(datasets) + 1:
                process_defaults(base_path)
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is_academic", action="store_true", help="Indicates if the dataset is classical/academic music")
    
    args = parser.parse_args()
    
    if args.dataset_dir and args.output_csv:
        extract_dataset_to_csv(args.dataset_dir, args.is_academic, args.output_csv)
    else:
        dataset_selection_menu()